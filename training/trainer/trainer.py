...
class Trainer(object):
    def __init__(
        ...
        if hasattr(model, 'set_trainer'):
            try:
                model.set_trainer(self)  # 将trainer实例传递给模型
            except Exception as e:
                logger.warning(f'Failed to bind trainer: {str(e)}')
                # 回退到兼容模式
                model.trainer = self
        else:
            # 兼容旧版本模型
    ...
    def _save_data_dict(self, data_dict, phase='train'):
        """安全保存data_dict"""
        try:
            save_path = os.path.join(
                self.config['log_dir'],
                f'{self.config["model_name"]}_{self.time_now}',
                phase,
                'data_dict_' + phase + '.pickle'
            )
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            with open(save_path, 'wb') as f:
                pickle.dump(data_dict, f)
            self.logger.info(f'data_dict saved to {save_path}')
        except Exception as e:
            self.logger.error(f'Failed to save data_dict: {str(e)}', exc_info=True)
            # 安静失败策略
            pass
